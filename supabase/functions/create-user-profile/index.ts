...
serve(async (req) => {
  ...
  try {
    ...
    switch (role) {
      ...
      case "doctor":
        tableName = "doctors";
        profileData = { user_id, full_name, phone, specialization, license_number };
        break;
      case "vendor":
        tableName = "vendors";
      ...
      case "delivery_rider":
        ...
        profileData = { user_id, full_name, phone, vehicle_type };
        break;
      case "kiosk_partner":
    ...
    return new Response(JSON.stringify({ message: `Profile created successfully for ${role}` }), {
      headers: { ...corsHeaders, "Content-Type": "application/json" },
      status: 200,
    });
